...
module.exports = function (options) {
	...
	async function getLastBlockHeight () {
		...
		return contract.methods['getHghestBlockNumber()']()
		 .send({ from: '', gas: options.GAS })
		 .then(blockHeight => { return blockHeight })
	...
	function submitTrx (blockHash, trxHash, from, to, amount) {
	 	contract.methods['submitTrx(bytes32,bytes32,string,string,uint256)'](new Buffer(blockHash), new Buffer(trxHash), from, to, Number(amount))
		 .send({ from: '', gas: options.GAS })
		 .then(receipt => { console.log(receipt); })
		 .catch( ex => { console.log(ex); });
